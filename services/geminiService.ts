...
function handleApiError(error: any): { finalImage: string | null; text: string } {
  console.error('Gemini API Error:', error);
  
  if (error.status === 'RESOURCE_EXHAUSTED' || error.code === 429) {
    return {
      finalImage: null,
      text: 'عذراً، تم تجاوز حد الاستخدام المجاني للذكاء الاصطناعي. يرجى المحاولة مرة أخرى لاحقاً أو ترقية خطتك.'
    };
  }
  ...
  if (error.status === 'INVALID_ARGUMENT' || error.code === 400) {
    return {
      finalImage: null,
      text: 'خطأ في مفتاح API أو في البيانات المرسلة. يرجى التحقق من الإعدادات.'
    };
  }
  
  return {
    finalImage: null,
    text: 'حدث خطأ أثناء معالجة الصورة. يرجى المحاولة مرة أخرى.'
  };
}
...
export async function applyStyleFromReference(
  ...
  try {
    ...
    const errorResult = handleApiError(error);
    return { base64Image: null, text: errorResult.text };
  }
}
...
export async function upscaleImage(
  ...
  try {
    ...
    const errorResult = handleApiError(error);
    return { upscaledImage: null, text: errorResult.text };
  }
}